...
def join_scenes(scenes, name="",scene_sep='···'):
    ...
    for scene in scenes:
        ...
        for i, line in enumerate(x3):
            ...
            tempx = get_comment_number(pre_line, line, next_line, name)
            if  tempx > commentmax:
                commentline = line
                commentmax = tempx
        if len(all_lines) > 0:
...
def get_comment_number(line_pre, line, line_next, novel_name):
    if not str.isnumeric(line.split(" ")[-1]):
        return 0
        ...
        for namea in all_speakers:
            for nameb in all_speakers:
                if namea != nameb:
                    ...
                    all_lines = join_scenes(join_adjacent_scenes(AB_chats[key]), name=base)
